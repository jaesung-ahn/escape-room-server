...
- [ ] RBAC 기본 구현 (Role enum, getAuthorities())
- [ ] RecommendationService N+1 제거
- [ ] ContentService 배치 이미지 로딩

### 1주차 (High Priority)

- [ ] GatheringService 분해 설계
- [ ] RuntimeException → ApiException 계층화
- [ ] 리소스 접근 권한 검증 강화
- [ ] 필수 인덱스 생성 (6개)
...
- [ ] `JwtTokenProvider.java:27` 하드코딩된 secret 제거
- [ ] `application.yml`에 jwt.secret 설정 추가
- [ ] `application-prod.yml` 환경변수 주입 설정
- [ ] `.env` 파일 또는 환경변수 설정 문서화
- [ ] 기존 토큰 무효화 및 재발급 로직 검토

**P0: RBAC 구현**
- [ ] `User.java`에 Role 필드 추가 (UserRole enum)
- [ ] UserRole enum 생성 (USER, ADMIN)
- [ ] `SecurityUser.getAuthorities()` 구현
- [ ] `SecurityConfig` @PreAuthorize 활성화
- [ ] 주요 엔드포인트에 @PreAuthorize 적용 (10개)
  - [ ] `/api/company` (POST) - ADMIN만
  - [ ] `/api/content` (POST, PUT, DELETE) - ADMIN만
  - [ ] `/api/review/approve` - ADMIN만
  - [ ] 기타 관리자 전용 API

**P1: 리소스 접근 권한 검증**
- [ ] `GatheringService.updateGathering()` 권한 검증 강화
- [ ] 모든 PUT 엔드포인트 소유권 검증
- [ ] 모든 DELETE 엔드포인트 소유권 검증
...
- [ ] `RecommendationService.getRecommendations()` 분석
- [ ] RecommendationRepository에 Fetch Join 쿼리 추가
- [ ] 테스트: 쿼리 개수 확인 (11-101개 → 2-3개)

대상 2: ContentService
- [ ] `ContentService.getContentSimpleModelsByContents()` 분석
- [ ] 이미지 ID 목록 수집
- [ ] `imageService.findByIdsIn()` 배치 조회 구현
- [ ] Map으로 변환하여 O(1) 조회
- [ ] 테스트: 쿼리 개수 확인 (N+1개 → 2개)

대상 3: GatheringService
- [ ] `GatheringService.getWaitingMember()` 분석
- [ ] 멤버 프로필 이미지 배치 로딩
- [ ] 테스트: 쿼리 개수 확인 (6개 → 2-3개)

대상 4: CommentService
- [ ] `CommentService` 대댓글 조회 분석
- [ ] @Fetch(FetchMode.SUBSELECT) 또는 별도 쿼리
- [ ] 테스트: 쿼리 개수 확인 (50개 → 6개)
...
- [ ] `api/application/exception/ApiException.java` (기본 클래스)
...
- [ ] `ApiExceptionHandler.java` 개선

**RuntimeException 제거**
- [ ] UserService: 4개 → ApiException
- [ ] MemberService: 1개 → ApiException
- [ ] ReviewService: 2개 → ApiException
- [ ] GatheringService: 2개 → ApiException

**에러 코드 중앙화**
- [ ] UserErrorCode 생성
- [ ] ContentErrorCode 생성
- [ ] ReviewErrorCode 생성
- [ ] GatheringErrorCode 확장
...
**인덱스 생성**
- [ ] `users.email` 인덱스
- [ ] `content(company_id, is_operated)` 복합 인덱스
- [ ] `gathering(user_id, deleted)` 복합 인덱스
- [ ] `review(content_id, is_approval)` 복합 인덱스
- [ ] `comment(gathering_id, parent_id)` 복합 인덱스
- [ ] `gathering_request(gathering_id, request_status)` 복합 인덱스
...
└── src/main/java/com/wiiee/server/api/
    ...
    └── application/exception/
        ├── ApiException.java (신규)
        ├── ResourceNotFoundException.java (신규)
        ├── UnauthorizedException.java (신규)
        ├── ForbiddenException.java (신규)
        ├── ValidationException.java (신규)
        ├── ConflictException.java (신규)
        └── InternalServerException.java (신규)
...
- [테스트 진행 현황](./TESTING_PROGRESS.md) - 테스트 현황
- [테스트 문제 해결](./TESTING_TROUBLESHOOTING.md) - 테스트 가이드
