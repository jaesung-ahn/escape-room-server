...
- [x] GatheringService 분해 구현
- [x] UserService 분해
- [x] QueryDSL Fetch Join 추가
- [x] Connection Pool 튜닝
- [x] 보안 헤더 설정
- [x] CORS 상세 설정
...
**설계**
- [x] 책임 분리 설계 (4개 서비스)
- [x] 인터페이스 및 메서드 시그니처 정의
- [x] 의존성 그래프 작성
- 참조: [GatheringService 분해 설계](./GatheringService%20분해%20설계.md)

**구현**
- [x] GatheringMemberService 생성 (100줄)
  - [x] addMember()
  - [x] getWaitingMember()
  - [x] removeMember()
  - [x] removeAllMembers()
  - [x] cancelJoinGathering()
- [x] GatheringRequestService 생성 (200줄)
  - [x] applyGathering()
  - [x] getGatheringRequestDetail()
  - [x] confirmGatheringRequest()
  - [x] cancelGatheringRequest()
- [x] GatheringNotificationService 생성 (110줄, 비동기)
  - [x] sendGatheringRequestPush()
  - [x] sendGatheringConfirmPush()
  - [x] @Async 설정
  - [x] @EnableAsync 추가 (ApiApplication)
- [x] GatheringService 정리 (545줄 → 214줄)
  - [x] 핵심 CRUD만 유지
  - [x] 분리된 서비스 의존성 주입

**테스트**
- [x] 기존 Acceptance 테스트 통과 확인 (전체 테스트 통과)
- [x] 서비스별 단위 테스트 작성 (GatheringServiceTest 6개, GatheringRequestServiceTest 10개, GatheringMemberServiceTest 4개, CommentServiceTest 5개, GatheringFavoriteServiceTest 2개, MemberServiceTest 1개)

### 2.2 UserService 분해

**설계**
- [x] 책임 분리 설계 (3개 서비스)
- 참조: [UserService 분해 설계](./UserService%20분해%20설계.md)

**구현**
- [x] AuthService 생성 (~100줄)
  - [x] kakaoLogin()
  - [x] signup()
  - [x] login()
  - [x] logout()
- [x] UserProfileService 생성 (~110줄)
  - [x] getMyPage()
  - [x] updateUser()
  - [x] updateUserSignupEtc()
  - [x] updateSettingUserInfo()
  - [x] updateUserPushInfo()
  - [x] updateUserPushNoti()
  - [x] checkNickname()
- [x] UserRecommendationService 생성 (~25줄)
  - [x] getMyWbtiRecommends()
- [x] UserService 정리 (235줄 → 38줄)

**테스트**
- [x] 기존 Acceptance 테스트 통과 확인 (전체 테스트 통과)
- [x] 서비스별 단위 테스트 작성 (AuthServiceTest 5개, UserProfileServiceTest 5개)
...
**방안 검토**
- [x] 현재 ModelMapper 사용 현황 분석 (api 1곳, admin 7곳)
- [x] MapStruct 도입 검토 → 변환의 70%가 복합 로직으로 자동화 ROI 낮음
- [x] 최종 방안 결정: ModelMapper 제거 + 기존 정적 팩토리 메서드 패턴(`from()`, `of()`)으로 표준화

**구현**
- [x] Form 클래스에 `from()` 정적 팩토리 메서드 추가 (5개)
  - [x] WbtiListForm.from(Wbti)
  - [x] EventForm.from(Event) — displayDate 포맷 변환 통합
  - [x] PushHistoryListForm.from(PushHistory)
  - [x] PushForm.from(PushHistory) — pushTypeCode 변환 내재화
  - [x] AppVersionDetailForm.from(AppVersion)
- [x] UserPushStatusDto.of(Boolean, Boolean, Boolean, Boolean) 팩토리 메서드 추가
- [x] Service 6개에서 ModelMapper 호출을 팩토리 메서드로 교체
- [x] ModelMapper Bean 설정 및 의존성 완전 제거 (api, admin)
- [x] UserProfileServiceTest ModelMapper Mock 제거, 전체 빌드 + 테스트 통과
...
**QueryDSL 최적화**
- [x] ContentCustomRepositoryImpl Fetch Join 추가
  - [x] findAllMainHotContent(): content, company fetchJoin
- [x] GatheringCustomRepositoryImpl Fetch Join 추가
  - [x] findAllByGatheringGetRequestDTO(): content, company fetchJoin
  - [x] findAllMyGathering(): content, company fetchJoin
- [x] count 쿼리 최적화

**Connection Pool 튜닝**
- [x] `application-prod.yml` hikari 설정
  - [x] maximum-pool-size: 20
  - [x] minimum-idle: 5
  - [x] idle-timeout: 300000
  - [x] max-lifetime: 1200000
...
- [x] `application.yml` Redis 설정 (docker-compose에서 환경변수로 주입)
- [x] RedisConfig 생성
- [x] @EnableCaching 설정

**캐시 구현**
- [x] WBTI 데이터 캐싱 (TTL: 1시간)
- [x] 추천 데이터 캐싱 (TTL: 15분)
- [x] 컨텐츠 목록 캐싱 (TTL: 5분)
- [x] FAQ 캐싱 (TTL: 1일)

**캐시 무효화**
- [x] 데이터 변경 시 캐시 무효화 로직
- [x] api: ContentService.createNewContent()에 @CacheEvict 적용
- [x] admin: Redis 의존성 및 설정 추가
- [x] admin: CacheEvictService 생성 (StringRedisTemplate 직접 삭제, 예외 허용)
- [x] admin: WbtiService.updateWbti() → wbti 캐시 무효화
- [x] admin: RecommendationService.updateRDT(), saveRDT() → recommendation 캐시 무효화
- [x] admin: ContentService.createContent(), updateContent(), saveContent(), deleteContentPrice() → content-hot 캐시 무효화

### 3.3 보안 강화

**보안 헤더**
- [x] SecurityConfig 보안 헤더 설정
...
**CORS 설정**
- [x] CorsConfigurationSource Bean 생성
- [x] allowedOrigins 환경변수화
- [x] allowedMethods 명시
- [x] maxAge 설정
...
- [x] build job 구성
  - [x] Docker 이미지 빌드
  - [x] GHCR (GitHub Container Registry) 푸시
- [ ] deploy job 구성 (선택)
...
**테스트**
- [x] docker-compose up 로컬 테스트
- [ ] 각 서비스 정상 동작 확인

### 4.3 테스트 커버리지

**단위 테스트 작성 (50개 완료)**
- [x] GatheringRequestServiceTest (10개)
- [x] GatheringServiceTest (6개)
- [x] GatheringMemberServiceTest (4개)
- [x] CommentServiceTest (5개)
- [x] GatheringFavoriteServiceTest (2개)
- [x] MemberServiceTest (1개)
- [x] AuthServiceTest (5개)
- [x] UserProfileServiceTest (5개)
- [x] ContentServiceTest (5개)
- [x] ReviewServiceTest (5개)
- [x] ContentFavoriteServiceTest (2개)

**JaCoCo 설정 (완료)**
- [x] `api/build.gradle` JaCoCo 플러그인 설정
- [x] jacocoTestReport 태스크 설정 (HTML + XML)
- [x] jacocoTestCoverageVerification 태스크 설정 (LINE 50%)
- [x] `test.finalizedBy jacocoTestReport` 자동 리포트 생성

**통합 테스트 확대 (43개 완료)**
- [x] 복잡한 비즈니스 로직 통합 테스트
- [x] 트랜잭션 경계 테스트

### 4.4 모니터링 및 로깅

**Actuator + Prometheus**
- [x] spring-boot-starter-actuator 의존성 추가
- [x] micrometer-registry-prometheus 의존성 추가
- [x] `application.yml` actuator 설정 (공통 endpoints 노출)
  - [x] health, info, metrics, prometheus 엔드포인트 노출
- [x] `application-local.yml` / `application-docker.yml` health show-details: always
- [x] `application-prod.yml` management.server.port: 9090 (별도 포트 분리)
- [x] SecurityConfig actuator permitAll + AntPathRequestMatcher 전환

**Grafana 대시보드**
- [x] Prometheus 설치 및 설정 (docker-compose, infra/prometheus/prometheus.yml)
- [x] Grafana 설치 및 설정 (docker-compose, infra/grafana/provisioning)
- [x] Spring Boot 대시보드 임포트 가능 (ID: 19004)
- [ ] 커스텀 메트릭 추가

**구조화된 로깅**
- [x] logback.xml → logback-spring.xml 전환
- [x] Logstash Encoder 의존성 추가 (logstash-logback-encoder:7.4)
- [x] prod 프로필 JSON 로그 포맷 설정 (LogstashEncoder)
- [x] docker 프로필 추가
- [x] 로그 레벨 환경별 설정
...
admin/
└── src/main/java/com/wiiee/server/admin/
    └── service/
        └── CacheEvictService.java (완료)

common/
└── src/main/java/com/wiiee/server/common/
    └── util/
        └── LogMaskingUtil.java (완료)
...
**Sprint 6 (2주)**
- Developer A & B: ~~ModelMapper 제거 완료~~ API 모듈 56개 Model 클래스 정적 팩토리 메서드 검증
...
**작성일**: 2025-12-29
**최종 업데이트**: 2026-03-11 (DTO 변환 로직 통일: ModelMapper 제거, 정적 팩토리 메서드 패턴 표준화)
