...
| 테스트 | 7/10 | Acceptance 테스트 86%, 단위 테스트 50개, JaCoCo 커버리지 측정 | MEDIUM |
...
#### 테스트 커버리지
- ~~단위 테스트 추가 (현재 0개 → 목표 50개+)~~ **완료** (50개 작성)
  - 서비스 레이어 단위 테스트 11개 파일, 50개 테스트 케이스
  - GatheringRequestServiceTest(10), GatheringServiceTest(6), GatheringMemberServiceTest(4), CommentServiceTest(5), AuthServiceTest(5), UserProfileServiceTest(5), ContentServiceTest(5), ReviewServiceTest(5), ContentFavoriteServiceTest(2), GatheringFavoriteServiceTest(2), MemberServiceTest(1)
- ~~JaCoCo 커버리지 목표: 70%~~ JaCoCo 설정 완료 (LINE 50% 검증, 자동 리포트 생성)
...
| 테스트 커버리지 | 10개 | 54개 (달성) | +440% |
| 코드 커버리지 | 약 40% | JaCoCo LINE 50% 검증 설정 | 측정 중 |
...
**작성일**: 2025-12-24
**최종 업데이트**: 2026-02-26 (단위 테스트 50개 + JaCoCo 반영)
